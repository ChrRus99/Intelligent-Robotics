...
pair<vector<PickUpObject>, vector<PickUpObstacle>> NodeA::send_request_to_node_B_service_server()
{
    // Create a service client for the human_node service
    ros::NodeHandle nh;
	ros::ServiceClient client = nh.serviceClient<custom_msgs::detected_objects>("/detected_objects_topic");

	// Create a service request
    custom_msgs::detected_objects srv;

    // Set the request to ready for receiving the object's ID 
    srv.request.is_ready = true;

    // Extract 
    vector<custom_msgs::PickUpObject> objects_msgs;
    vector<custom_msgs::PickUpObstacle> obstacles_msgs;
